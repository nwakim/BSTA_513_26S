---
title: "Lesson 11: Numerical Problems"
author: "Nicky Wakim"
...
format: 
  revealjs:
    ...
    width: 1955
    height: 1100
    footer: "Lesson 11: Numerical Problems"
...
## 2. Collapse the categories of predictor: first check if close enough!

Just to be safe: I'd check the Fisher's Exact test for the 2x2 table of groups 2 and 3:
...
ex1_1 = ex1 %>% 
  filter(x != "One") %>%
  mutate(x = factor(x, levels = c("Two", "Three")), 
         outcome = factor(outcome, levels=c("1", "0")))

fisher.test(x = table(ex1_1$x, ex1_1$outcome))
```
:::
::: {.column width="30%"}

::: box2
Remember, I'm using Fisher's Exact test because I have a zero cell count! If the expected cell count was > 5, I could use a difference in proportions or Chi-squared test. 
...
**NOT APPROPRIATE, but let's run through the thought experiment**
...
pred_link = data.frame(x = newdata, Pred = pred$fit, LL_CI1, UL_CI1) 
```
...
#| fig-height: 2.5
#| code-fold: true
#| code-summary: Plotting fitted log-odds

ggplot() + 
      labs(y = "Log-odds of Outcome") +
      scale_fill_manual(values=c("#D6295E", "#ED7D31")) +
      geom_point(data = pred_link, aes(x = x, y=Pred), size=3) +
      geom_errorbar(data = pred_link, aes(x = x, y=Pred, ymin = LL_CI1, ymax = UL_CI1), width = 0.25)
```
...
-   Outcomes of 0 and 1 are completely separated by `x1`
    
    -   If `x1 > 4` then outcome is 1
    -   If `x1 < 4` then outcome is 0
...
-   Collapse categorical variables in a meaningful way
    ...
    -   Other reasons why you MUST stay with logistic regression
...
-   Firth logistic regression (best option in terms of statistical properties)
...
![](images/firth.png){.absolute bottom=8 right=0 width="150"}
