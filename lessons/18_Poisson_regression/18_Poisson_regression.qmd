---
title: "Lesson 18: Introduction to Poisson Regression"
author: "Nicky Wakim"
...
format: 
  revealjs:
    ...
    width: 1955
    height: 1100
    footer: "Lesson 18: Poisson Regression"
...
![](./images/horseshoe_crab.jpeg){width="375" fig-align="center"}
:::
...
$$\log(\mu(X)) = \log(\lambda(X)) = \beta_0 + \beta_1 X$$
...
-   In simple Poisson regression: $$\log(\mu(X)) = \log(\lambda(X)) = \beta_0 + \beta_1 X$$

-   When $X$ is a binary variable: How do we interpret $\beta_1$?

    -   When $X=0$: $$\log(\mu(X = 0)) = \beta_0 + \beta_1 \cdot 0 = \beta_0$$
        -   $\mu(X = 0) = \exp (\beta_0)$: the mean count or rate of $Y$ when $X=0$
    -   When $X=1$: $$\log(\mu(X = 1)) = \beta_0 + \beta_1 \cdot 1 = \beta_0 + \beta_1$$
        -   $\mu(X = 1) = \exp (\beta_0 + \beta_1)$: the mean count or rate of $Y$ when $X=1$

## Parameter Interpretation: Binary $X$ (2/2)

-   When $X$ is a binary variable: How do we interpret $\beta_1$?
    -   By subtraction, we have $$\beta_1 = \beta_0 + \beta_1 - (\beta_0) = \log(\mu(X = 1)) -  \log(\mu(X = 0)) = \log \left( \dfrac{\mu(X = 1)}{\mu(X = 0)} \right)$$
...
-   What if we have data where each observation has different period of time?
...
$$\begin{aligned} \log(\lambda(X)) = & \beta_0 + \beta_1 X \\
\log(\lambda(X)) = \log\left(\dfrac{\mu(X)}{t(X)}\right) = \log(\mu(X)) - \log(t(X)) = &\beta_0 + \beta_1 X \\
\log(\mu(X)) = & \log(t(X)) + \beta_0 + \beta_1 X\\ \end{aligned}$$

## Including an offset when we have different follow-up times

-   That is, to incorporate the different lengths in the model,

$$\log(\mu(X)) =  \log(t(X)) + \beta_0 + \beta_1 X$$
