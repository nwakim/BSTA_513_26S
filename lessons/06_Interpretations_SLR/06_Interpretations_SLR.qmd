---
title: "Lesson 6: Interpretations and Visualizations of Odds Ratios"
author: "Nicky Wakim"
...
format: 
  revealjs:
    ...
    width: 1955
    height: 1100
    footer: "Lesson 6: Interpretations and Visualizations of Odds Ratios"
...
-   Recall that we fit a simple logistic regression for late stage breast cancer diagnosis using the predictor, age:

```{r}
bc_reg = bc %>% glm(formula = Late_stage_diag ~ Age_c, family = binomial)
...
-   $\beta_0$: log-odds of $Y=1$ when $X$ is 0
-   $\beta_1$: increase in log-odds of $Y=1$ for every 1 unit increase in $X$
:::
:::
:::
...
:::
::: prop-cont
$$\text{logit}(\widehat{\pi}(X)) = \widehat{\beta}_0 + \widehat{\beta}_1 \cdot X$$
...
-   $\widehat{\beta}_0$: The estimated log-odds is -0.989 for someone who is `r round(mean_age, 2)` years (95% CI: -1.035, -0.944)

-   $\widehat{\beta}_1$: The estimated increase in log-odds is 0.057 for every 1 year increase in age (95% CI: 0.051, 0.063).

-   What does a log-odds even mean?? Or an increase in log-odds??

-   We will need to calculate the **odds ratio** and its confidence interval

    -   Then we will visualize the odds ratio
:::
...
-   Thus, $$\begin{aligned} \widehat{\beta}_1 & =  \text{logit}(\widehat{\pi}(X = x+1)) - \text{logit}(\widehat{\pi}(X = x)) \\
    ...
    \text{exp}\big[\widehat{\beta}_1\big] & =  \text{exp}\left[\text{log}\left(\dfrac{\widehat{\text{odds}}_{X=x+1}} {\widehat{\text{odds}}_{X=x}}\right) \right] \\ 
    \text{exp}\big[\widehat{\beta}_1\big] & =  \dfrac{\widehat{\text{odds}}_{X=x+1}} {\widehat{\text{odds}}_{X=x}} \\ 
    \end{aligned}$$
:::
:::
...
:::
::: axiom-cont
$$\widehat{\beta}_1 = 0.057$$

One year increase in age is associated with expected 0.057 increase in [**log-odds**]{style="color: #D6295E;"} of receiving a late stage breast cancer diagnosis
:::
:::
...
:::
::: proof-cont
$$\exp\left({\widehat{\beta}}_1\right) = 1.06$$

- One year increase in age is associated with expected 1.06 [**times the odds**]{style="color: #ED7D31;"} of receiving a late stage breast cancer diagnosis

    - Can also say: For every one year increase in age, the expected odds of late stage breast cancer diagnosis increases 1.06 times. 

-   Can also subtract 1 from the odds ratio and multiply by 100 to obtain the percentage change in odds for 1-unit increase.

    -   1-year increase in age is associated with an [6% increase in the predicted odds]{style="color: #ED7D31;"} of late stage diagnosis
:::
:::
...
```{r}
bc_reg = bc %>%
  glm(formula = Late_stage_diag ~ Age_c, family = binomial)
```
...
For every one year increase in age, there is an expected `r round(100*(tidy_bc_reg$estimate[2]-1), 2)`% increase in the odds of late stage breast cancer diagnosis (95% CI: `r round(100*(tidy_bc_reg$conf.low[2]-1), 2)`%, `r round(100*(tidy_bc_reg$conf.high[2]-1),2)`%).
...
**Conclusion:** For every 10-year increase in age, the expected odds of late stage breast cancer diagnosis increases 1.77 times (95% CI: 1.66, 1.88).
...
-   The model implies the additional odds of late stage breast cancer diagnosis for a 40 year-old compared to a 30 year-old is the same as the additional odds of late stage breast cancer diagnosis for a 60 year-old compared to a 50-year-old
...
- See Lesson 8 and Lesson 14 in our [BSTA 512/612 class](https://nwakim.github.io/BSTA_512_26W/schedule.html)
...
-   Thus, $$\begin{aligned} \widehat{\beta}_0 & = \text{logit}(\widehat{\pi}(X=0)) \\
    \text{exp}\big[\widehat{\beta}_0\big] & = \text{exp}\big[\text{logit}(\widehat{\pi}(X=0))\big] \\ 
    \text{exp}\big[\widehat{\beta}_0\big] & = \text{exp}\Bigg[\text{log}\Bigg(\dfrac{\widehat{\pi}(X=0)}{1-\widehat{\pi}(X=0)}\Bigg)\Bigg] \\ 
    \text{exp}\big[\widehat{\beta}_0\big] & = \dfrac{\widehat{\pi}(X=0)}{1-\widehat{\pi}(X=0)} \\ 
    \end{aligned}$$
...
-   We are fitting the population simple logistic regression model: $$\text{logit}\left(\pi(X) \right) = \beta_0 + \beta_1 \cdot I(X=1)$$
...
-   The logit difference is $\beta_1$ for binary independent variable
    ...
    -   $\beta_1$ represents the change/difference in the logit for $X=1$ vs. $X=0$
        - AKA log odds ratio comparing $X=1$ to $X=0$

 

-   It will be much easier to understand if we can interpret the coefficient using odds ratio (OR)

## Binary predictor: How do we interpret the coefficient? (1/2)
...
## Binary predictor: How do we interpret the coefficient? (2/2)
...
## Review of Odds Ratio {visibility="hidden"}
...
## How does this relate to a 2x2 table? {visibility="hidden"}
...
## Example: Binary age and Late Stage Diagnosis (1/4)
...
-   Two options to calculate this value:
    ...
    -   [Option 1:]{style="color:#ED7D31; font-weight: bold;"} Calculate $\widehat{OR}$ from 2x2 contingency table
        ...
        -   Try at home: Refer to [Lesson 3](https://nwakim.github.io/S2024_BSTA_513/lectures/03_Meas_Assoc_CT/03_Meas_Assoc_CT.html#/title-slide) for this process
...
## Example: Binary age and Late Stage Diagnosis (2/4)
...
age_bin_glm = bc3 %>% glm(formula = Late_stage_diag ~ Age_binary, family = binomial)
```

## Example: Binary age and Late Stage Diagnosis (3/4)
...
## Example: Binary age and Late Stage Diagnosis (4/4)
...
-   $\beta_1$ represents the change/difference in the log-odds for $X=b$ vs. $X=a$
      - AKA log odds ratio comparing $X=b$ to $X=a$
...
- Remember, as soon as we fit the regression model, we talk about the "expected odds" or "estimated odds"
...
- In this class, we will try our best to pick a reference from our own knowledge...
    - **but** in general this is a decision I make with someone who has expertise in the field of study

## Example: Late stage diagnosis and race and ethnicity

::: columns
::: {.column width="40%"}
-   We chose Non-Hispanic White individuals as reference group
...
- Something else to consider: if your reference group does not have a lot of individuals, then you may not see any statistical significance! 
...
$$
\begin{aligned}
\text{logit}\left(\pi(X) \right) = & \beta_0 + \beta_1 \cdot I(X=\text{Hispanic-Latino}) + \\ & \beta_2 \cdot I(X=\text{NH American Indian/Alaskan Native}) + \\ & \beta_3 \cdot I(X=\text{NH Asian/Pacific Islander}) + \\ & \beta_4 \cdot I(X=\text{NH Black})
\end{aligned}
$$
...
## Reference: Using `estimable()` in [Option 2]{style="color:#D6295E; font-weight: bold;"}

- We could use `estimable()` to calculate this linear combination of coefficients ([see BSTA 512 Lesson](https://nwakim.github.io/BSTA_512_W25/lessons/12_Interactions_02/12_Interactions_02.html#/we-can-use-r-and-estimable-to-find-the-estimate-and-ci))

```{r}
library(gmodels)
RE_glm %>% estimable(
                   c("(Intercept)"                                     = 0,   # beta0
                     "Race_EthnicityHispanic-Latino"                   = 0,   # beta1
                     "Race_EthnicityNH American Indian/Alaskan Native" = 0,   # beta2
                     "Race_EthnicityNH Asian/Pacific Islander"         = 1,   # beta3
                     "Race_EthnicityNH Black"                          = -1), # beta4
                   conf.int = 0.95) %>% 
  exp(.)
```
...
```{r}
bc3 = bc %>% 
  mutate(Race_Ethnicity = fct_relevel(Race_Ethnicity, "NH Black"))
...
ggplot(data=RE_tidy, aes(y=label, x=estimate, xmin=conf.low, xmax=conf.high)) + 
  ...
  theme_classic() +
  labs(x = "Estimated OR (95% CI)", y = "Race and ethnicity", 
       title = "Odds ratios of late stage breast cancer diagnosis") +
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 25), title = element_text(size = 24))
```
