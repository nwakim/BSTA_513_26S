---
title: "Lesson 8: Tests for GLMs using Likelihood function"
author: "Nicky Wakim"
...
format: 
  revealjs:
    ...
    width: 1955
    height: 1100
    footer: "Lesson 8: Tests for GLMs using Likelihood function"
...
- Wald test measures the "difference" between $\beta_1=0$ and $\widehat{\beta}_1$ on the x-axis
...
::: proof-cont
I don't want us to get fixated on this interpretation. This is more to introduce the process, BUT it's MUCH better to interpret the coefficient in terms of OR.
:::
:::::
::::::
::::::::
...
-   Score test **does not require the computation of MLE** for $\beta_1$, while both likelihood test and Wald test do
...
- Translates to the **slope of the tangent** line at $\beta_1=0$

 

- In my experience, this is the test I've seen the least: won't show an example with it

## Score test {background-image="./images/title_yell.png" background-size="2500px 150px" background-position="top"}

- Score test measures the "difference" between $\beta_1=0$ and $\widehat{\beta}_1$ by measuring slope of the tangent line at $\beta_1=0$
...
-   Likelihood ratio test (LRT) answers the question:
...
-   **Very similar to the F-test**: test significance by comparing different, nested models

 

-   Important: LRT can be used conduct hypothesis tests for a **single coefficient** or **multiple coefficients** 
...
-   Test statistic for LRT: $$G=-2ln\left[\frac{\text{likelihood without } x}{\text{likelihood with } x}\right]=2ln\left[\frac{l\left({\widehat{\beta}}_0,{\widehat{\beta}}_1\right)}{l({\widehat{\beta}}_0)}\right]$$
...
So our LRT statistic is: $$G=D\left(\text{model without } x\right)-D\left(\text{model with } x\right)$$ 

::: columns
::: {.column width="50%"}
-   Deviance for model without $x$: $$D_{\text{without x}}=-2[L_{\text{model without x}} - L_{\text{saturated}}]$$
:::
::: {.column width="50%"}
-   Deviance for model with $x$: $$D_{\text{with x}}=-2[L_{\text{model with x}} - L_{\text{saturated}}]$$
...
G&=2ln\left[\frac{l\left({\widehat{\beta}}_0,{\widehat{\beta}}_1\right)}{l({\widehat{\beta}}_0)}\right]
\end{aligned}$$
...
- We can use the `pchisq()` if you calculate $G$ first, but there's an R function to run an LRT 

## Likelihood ratio test {background-image="./images/title_green.png" background-size="2500px 150px" background-position="top"}

- Likelihood ratio test measures the **difference of likelihoods** (y-axis) between $\beta_1=0$ and $\widehat{\beta}_1$
...
# anova(bc_int, bc_age, test = "Chisq") # another option to run it
```
...
-   For testing significance of single covariate coefficient:
    ...
    -   [Score test]{style="background-color:#F9E7B8;"}
        ...
        -   Does not need to estimate two models (LRT does), but does not incorporate info from MLE
