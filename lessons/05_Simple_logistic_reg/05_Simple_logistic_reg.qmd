...
-   Our analysis will not be new, but this kind of work has shed light on the importance of focused research on people of color to better serve people of color who develop breast cancer
    ...
    - More public health work focused on access to healthcare and patient-doctor relationships
...
-   [Logistic regression]{style="color:#F6C342; font-weight: bold;"} **models can handle multiple covariates that are continuous or categorical**
:::
...
-   Because the [expected value is a weighted average](https://nwakim.github.io/F24_EPI_525/lessons/05_RVs_Binomial/05_RVs_Binomial.html#/expectation-of-random-variables), we can say: $$\begin{aligned} E(Y|X) & = P(Y=1|X) \cdot 1 + P(Y=0|X) \cdot 0 \\ & = P(Y=1|X) \cdot 1 \\ & = P(Y=1|X) \\ & = \pi(X) \end{aligned}$$
...
-   What assumptions do our data need to meet in order to use OLS?
-   Let’s review OLS assumptions!
...
-   Assumptions of the linear regression model:
    ...
    -   Normality and homoscedasticity assumption for residuals ($\epsilon$):
        ...
        -   Normality: residuals are normally distributed ($\epsilon \sim N(0, \sigma^2)$)
...
## Violated: Homoscedasticity / equal variance
...
-   Here we will focus on the GLMs for **categorical/count data**
    ...
    -   [Log-binomial]{style="color:#ED7D31; font-weight: bold;"} can be used to focus on prevalence/risk ratio
...
-   The random component specifies the response variable $Y$ and identifies a probability distribution for it
...
-   If $\mu = E(Y)$, then the link function specifies a function $g(.)$ that relates $\mu$ to the linear predictors
    ...
    -   AKA: link function connects the random component with the systematic component

- So the general form of a regression equation looks like: $$g\left(\mu\right)=\beta_0+\beta_1X_1+\ldots+\beta_kX_k$$ and $g\left(\mu\right)$ is the **placeholder for the transformation** we make to mean $\mu$

- For logistic regression, $g\left(\mu\right) = \text{logit} (\mu)$, so the regression equation is $\text{logit} (\mu)=\beta_0+\beta_1X_1+\ldots+\beta_kX_k$
- For linear regression, $g\left(\mu\right) = \mu$ and so the regression equation is $\mu = \beta_0+\beta_1X_1+\ldots+\beta_kX_k$

-   Can also think of this of equation with inverse of $g$: $$\mu=g^{-1}\left(\beta_0+\beta_1X_1+\ldots+\beta_kX_k\right)$$
...
-   [Maximum likelihood]{style="color:#5B9BD5; font-weight: bold;"}: yields estimates for the unknown parameters that *maximize the probability of obtaining observed set of data*

    -   In linear regression, this leads to [least squares estimation (OLS)](https://nwakim.github.io/BSTA_512_W25/lessons/03_SLR/03_SLR.html#/learning-objectives-4)
        - But we can't use OLS in all types of regression
...
4. Find parameter estimates that maximize log-likelihood: MLEs
...
$$\begin{aligned}
\text{if } Y=1 &:& P(Y=1)^1P(Y=0)^0\\
\text{if } Y=0 &:& P(Y=1)^0P(Y=0)^1
\end{aligned}$$
...
- The likelihood for the whole sample is the likelihood for each individual multiplied together (result of independence assumption)
:::
...
-   The log-likelihood function is: $$\begin{aligned}L\left(\beta_0,\beta_1\right) &=\log{\left(l\left(\beta_0,\beta_1\right)\right)} \\ & = 
...
-   Such equations are called likelihood equations: result of differentation and setting to 0
...
-   In logistic regression, there is no "closed form" solution to the above equations
    - Meaning we cannot manipulate the math to find the solution
