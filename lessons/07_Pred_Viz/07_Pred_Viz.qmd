...
3. Calculate confidence interval
    ...
    a.   **Option 1:** Using logit scale then convert to probability scale (can use even if Normal approximation criteria not met)
    b.   **Option 2:** Using Normal approximation directly on probability scale
...
::: columns
::: column
- Using definition of inverse logit
```{r}
(exp(pred_link)/(1+exp(pred_link))) %>% 
  round(., digits=3)
```
:::
::: column
- Using inverse logit in `boot` library
```{r}
inv.logit(pred_link) %>% 
  round(., digits=3)
```
...
-   [Predicted *probability*]{style="color:#4472C4"} is **NOT** our [predicted *outcome*]{style="color:#D6295E"}

    -   We **cannot interpret** it as the predicted $Y$ for individuals with certain covariate values

    -   Example: our [predicted probability]{style="color:#4472C4"} does not tell us if one individual will or will not be diagnosed with late stage breast cancer

 

-   The [predicted probability]{style="color:#4472C4"} is the estimate of the **mean** (i.e., proportion) of individuals at a certain age who are diagnosed with late stage breast cancer

-   We can use the [predicted/estimated probability]{style="color:#4472C4"} to **predict** the [outcome]{style="color:#D6295E"}

## Predicted outcome

-   Typically, the [predicted probability]{style="color:#4472C4"} is the **most important thing to use in a clinical setting**

 

-   If you ever need to **predict the outcome itself** (from logistic regression with binary outcome):

    -   Remember that the [predicted probability]{style="color:#4472C4"} can be used in a Bernoulli (or Binomial with $n=1$) distribution to find the [predicted *outcome*]{style="color:#D6295E"}
...
```{r}
prob_stage_CI = ggplot(data = with_CI, aes(x = Age)) +
...
::: {.column width="57%"}
::: hl3
[These two visualizations will be helpful when we discuss interactions!]{style="font-size:1.5em;"}
:::
:::
:::
...
# Reference: predicted prob plot for categorical predictor

## We can add the confidence intervals (1/3)

- Model with LSBC and race/ethnicity

```{r}
bc_re = glm(Late_stage_diag ~ Race_Ethnicity, data = bc, family = binomial)
```

```{r}
newdata2 = data.frame(Race_Ethnicity = levels(bc$Race_Ethnicity))
pred2 = predict(bc_re, newdata = newdata2, se.fit = T, type = "link")
LL_CI1 = pred2$fit - qnorm(1-0.05/2) * pred2$se.fit
UL_CI1 = pred2$fit + qnorm(1-0.05/2) * pred2$se.fit

with_CI = data.frame(Race_Ethnicity = levels(bc$Race_Ethnicity), 
                     pred = inv.logit(pred2$fit), 
                     LL = inv.logit(LL_CI1), 
                     UL = inv.logit(UL_CI1))
```
...
```{r}
prob_stage_CI = ggplot(data = with_CI, aes(x = Race_Ethnicity)) +
  geom_errorbar(aes(ymin = LL, ymax = UL), size = 1, width = 0.2) + 
  geom_point(aes(y = pred), size = 4, color = "#70AD47") +
