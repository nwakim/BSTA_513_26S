...
-   And we assume that every level of the potential confounder, there is parallel 
slopes
...
-   We can incorporate interactions into our model through product terms: $$\text{logit}\big(\pi(\bf{X}) \big)= \beta_0 + \beta_1X_{1}+ \beta_2X_{2} +
    \beta_3X_{1}X_{2}$$
...
-   [**If age interacts with sex**]{style="color:#5B9BD5"}, the distance between $l_3$ and $l_1$ is the log odds ratio for sex at specific values of age 
...
::: definition
::: def-title
Multi-level categorical & continuous variables
:::
...
-   Is age an effect modifier?
    ...
    -   We can use LRT (or Wald test bc only one coefficient for interaction)
...
library(gmodels)
glow_m3 %>% estimable(
               cbind("(Intercept)"                = 0,   # beta0
                     "priorfracYes"               = 1,   # beta1
                     "age_c"                      = 0,   # beta2
                     "priorfracYes:age_c"         = (55:90)-69), 
                   conf.int = 0.95) %>% 
  exp(.) %>%
  head(5)
```
...
## Calculate odds ratios across values (`predict()`)
...
::: columns
::: column
For individuals 69 years old, the estimated odds of a new fracture for individuals *with prior fracture* is 2.72 times the estimated odds of a new fracture for individuals *with no prior fracture* (95% CI: 1.70, 4.35). As seen in @fig-plots-1, the estimated odds ratio of a new fracture when comparing prior fracture status decreases with age, indicating that the effect of prior fractures on new fractures decreases as individuals get older. In @fig-plots-2, it is evident that for both prior fracture statuses, the predicted probability of a new fracture increases as age increases. However, the predicted probability of new fracture for those without a prior fracture increases at a higher rate than that of individuals with a prior fracture. Thus, the predicted probabilities of a new fracture converge at age [insert age here]. 
:::
::: column
```{r}
#| label: fig-plots
...
#save(pred_glow2, glow_m3, prior_age, file = here("lessons", "12_Assessing_fit/data", "glow_results.Rda"))
