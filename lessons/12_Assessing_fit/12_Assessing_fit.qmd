---
title: "Lesson 12: Assessing Model Fit (and comparing non-nested models)"
author: "Nicky Wakim"
...
3.  Use the AUC-ROC to determine how well model predicts binary outcome
...
- Fitted model with interactions: $$\begin{aligned} \text{logit}\left(\widehat\pi(\mathbf{X})\right) & = \widehat\beta_0 + \widehat\beta_1  \cdot I(\text{PF}= \text{``Yes"}) + \widehat\beta_2  \cdot Age + \widehat\beta_3  \cdot I(\text{PF} = \text{``Yes"}) \cdot Age \\  
...
## Assessing model fit: nested vs unnested models

::::::::::: columns
:::::: column
::::: axiom
::: axiom-title
Nested models and LRT
:::

::: axiom-cont
- If we have [two nested models]{style="color:#D6295E; font-weight: bold;"}, we can use likelihood ratio test to compare the fit of the two models

  - Nested models: one model's variables are a subset of the other

![](./images/nested_models.png){fig-align="center" width="478"}
:::
:::::
...
::::: proof1
::: proof-title
Unnested models and comparisons
:::

::: proof-cont
- If we have [two unnested models]{style="color:#ED7D31; font-weight: bold;"}, we cannot use likelihood ratio test to compare the fit of the two models

  - Non-nested models: can have overlapping variables, but not a complete subset of variables
  
![](./images/unnested_models.png){fig-align="center" width="590"}
:::
:::::
...
- The model fits the data well if
  ...
  - [Summary measurements of model fit]{style="color:#5B9BD5; font-weight: bold;"} tell us good fit
    - Aka: Summary measures of the distance between the predicted/estimated/fitted and observed Y are small

    - Today's lecture!!
  - No major issues in [model diagnostics]{style="color:#70AD47; font-weight: bold;"}, observations fit into model and our assumptions
...
- In logistic regression model, we estimate $\pi(\mathbf{X}) = P(Y=1|\mathbf{X})$
  ...
  - Predicted probability, $\widehat\pi(\mathbf{X})$, is between 0 and 1 for each subject
...
- Pearson residual for jth covariate pattern is: $$r\left(Y_j,{\hat{\pi}}_j\right)=\frac{(Y_j-m_j{\hat{\pi}}_j)}{\sqrt{m_j{\hat{\pi}}_j(1-{\hat{\pi}}_j)}}=\frac{(Y_j-{\hat{Y}}_j)}{\sqrt{{\hat{Y}}_j(1-{\hat{\pi}}_j)}}$$
...
| Bayesian information criterion (BIC) | $BIC = -2 \cdot \text{log-likelihood} + q\text{log}(n)$ | `BIC(model_name)` |
...
```{r}
#| echo: true
model1 = glow %>% glm(formula = fracture ~ age_c + height, family = binomial)
model2 = glow %>% glm(formula = fracture ~ priorfrac + armassist + smoke, family = binomial)
model3 = glow %>% glm(formula = fracture ~ priorfrac + age_c + priorfrac*age_c, family = binomial)
predicted1 = predict(model1, glow, type="response")
predicted2 = predict(model2, glow, type="response")
predicted3 = predict(model3, glow, type="response")

fit_stats = data.frame(Model = c("Model 1: age + height", "Model 2: prior fracture + arm assist + smoking status", "Model 3: interaction with prior fraction + age"), 
                       AUC_ROC = c(round(auc(glow$fracture, predicted1),4), round(auc(glow$fracture, predicted2),4), round(auc(glow$fracture, predicted3),4)), 
                       AIC = c(AIC(model1), AIC(model2), AIC(model3)), 
                       BIC = c(BIC(model1), BIC(model2), BIC(model3)))

colnames(fit_stats) = c("Model", "AUC-ROC", "AIC", "BIC")
...
- We went over model fit statistics a little in BSTA 512/612 in [Lesson 13](https://nwakim.github.io/BSTA_512_W25/lessons/13_Model_selection/13_Model_selection.html?q=aic#/learning-objectives-3)
