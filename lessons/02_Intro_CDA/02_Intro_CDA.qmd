...
3.  Estimate a single proportion

4.  Estimate a difference in proportions
...
2.  Check that each cell has at least 5 individuals
3.  Construct 95% confidence interval
4.  Write interpretation of estimate
...
-   McNemar’s test should be used if data (in a 2x2 table) are from a matched pairs study
...
- Study on new postoperative neuropathy after receiving gender affirming care ([Study link](https://pmc.ncbi.nlm.nih.gov/articles/PMC10686520/))
...
neuro_table = table(neuropathy$EBL_g250ml, neuropathy$Postop_neuro)
...
::: {.column width="60%"}
4.  Calculate the test statistic and p-value for Fisher Exact test
...
1. The proportions of successes tend to increase as ordinal value of the explanatory variable increases

$$p_1 \leq p_2 \leq ... \leq p_C$$

OR

2. The proportions of successes tend to decrease as ordinal value of the explanatory variable increases
...
1. [**Check assumptions**]{style="color:#70AD47"}: expected cell counts is greater than 5. 
...
4.  Calculate the [**test statistic**]{style="color:#70AD47"} and [**p-value**]{style="color:#70AD47"} for Cochran-Armitage test in R
...
::: column
4.  Calculate the test statistic and p-value for Cochran-Armitage test in R

```{r}
library(DescTools)
CochranArmitageTest(bscancer, alternative = "one.sided")
```
...
-   When both variables are ordinal, we can conduct Mantel-Haenszel test of trend for linear association
    - Test for independence when both variables are ordinal

-   Mantel-Haenszel test for linear trend is suitable for any R x C contingency tables with two ordinal variables

- More info on the test statistic and its Chi-squared distribution on the [Penn State site](https://online.stat.psu.edu/stat504/Lesson04#mantel-haenszel-test-for-linear-trend)
...
4.  Calculate the [**test statistic**]{style="color:#70AD47"} and [**p-value**]{style="color:#70AD47"} for Mantel-Haenszel test in R
...
::: column
4.  Calculate the test statistic and p-value for Mantel-Haenszel test in R
...
- [Review from my EPI 525 class in F25](https://nwakim.github.io/F24_EPI_525/schedule.html)
