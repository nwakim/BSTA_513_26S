...
-   Then we can define risk/prevalence: the proportion of "successes"

    -   Risk/proportion/prevalence of successful response for explanatory Group 1: $\widehat{\text{Risk}}_1 = \widehat{p}_1 = \dfrac{n_{11}}{n_1}$
    -   Context and study design will determine the name
...
::::: columns
::: {.column width="60%"}
-   Point estimate: $$\widehat{RD} = \widehat{p}_1 - \widehat{p}_2 = \dfrac{n_{11}}{n_1} - \dfrac{n_{21}}{n_2}$$
...
-   95% Wald confidence interval for $\widehat{RD}$ (given expected cell counts \> 5):
...
## SHS Example: Risk Difference (1/6)
...
1.  Check expected cell counts \> 5
2.  Compute the risk difference
3.  Compute 95% confidence interval
4.  Interpret the estimate

## SHS Example: Risk Difference (2/6)
...
::::: columns
::: column
Create the table and look at it:
...
## SHS Example: Risk Difference (3/6)
...
2.  Compute the risk difference $$\widehat{RD}={\hat{p}}_1 - {\hat{p}}_2=\frac{n_{11}}{n_1}-\frac{n_{21}}{n_2}=\ \frac{198}{532}\ - \frac{128}{1132}=`r round(198/532, 4)`−`r round(128/1132, 4)`=`r round(198/532 - 128/1132, 4)`$$

## SHS Example: Risk Difference (4/6)
...
## SHS Example: Risk Difference (5/6)
...
2/3. Compute risk difference and 95% confidence interval

```{r}
fmsb::riskdifference(198, 128, 532, 1132)
```

## SHS Example: Risk Difference (6/6)
...
The diabetes diagnosis risk difference between impaired and normal glucose tolerance is 0.2591 (95% CI: 0.2141, 0.3041). Since the 95% confidence interval does not contain 0, we have sufficient evidence that the risk of diabetes diagnosis for people with impaired versus normal glucose tolerance is different.
...
-   Point estimate: $$\widehat{RR}=\dfrac{\hat{p}_1}{\hat{p}_2} = \dfrac{n_{11}/n_1}{n_{21}/n_2}$$
    ...
    -   Range: $[0, \infty]$
        ...
        -   if $\widehat{p}_1=0$ then $\widehat{RR}=0$ regardless of $\widehat{p}_2$
        -   if $\widehat{p}_2=0$ then $\widehat{RR}=\infty$ regardless of $\widehat{p}_1$
:::

::: {.column width="50%"}
![](./images/ct.png){fig-align="center"}

What do values of risk ratios mean?

| Risk Ratio | Clinical Meaning |
|--------------------|----------------------------------------------------|
| $\widehat{RR} < 1$ | Risk is smaller in group 1 than in group 2 |
| $\widehat{RR} = 1$ | Risks are the same (explanatory and outcome are independent) |
| $\widehat{RR} > 1$ | Risk is greater in group 1 than in group 2 |
...
1.  Check expected cell counts \> 5
2.  Compute the relative risk
3.  Find confidence interval of log RR
4.  Convert back to RR
5.  Interpret the estimate

## SHS Example: Relative Risk (2/7)
...
**2/3/4. Compute risk ratio and 95% confidence interval**
...
3.  Find confidence interval of log OR
4.  Convert back to OR
5.  Interpret the estimate

## SHS Example: Odds Ratio (2/7)
...
2/3/4. Compute OR and 95% confidence interval
